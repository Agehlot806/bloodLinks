...
const LabMap = ({ route }) => {
    ...
    useEffect(() => {
        getOneTimeLocation();
        getLocationApi();
        console.log('param data get lat', paramValuelat)
        console.log('param data get long', paramValuelng)
    }, []);
    useEffect(() => {
        getOneTimeLocation();
    ...
    const getLocationApi = () => {
        fetch('https://bloodlinks.in/lab_list')
            ...
            .then((response) => {
                console.log('response api data Labs Map list----->>>>>', response)
    ...
    return (
        <View style={styles.container}>
            <MapView
                ...
                <Polyline
                    coordinates={[state, chibaRegion]} //specify our coordinates
                    strokeColor={"#93121B"}
                    strokeWidth={10}
                    lineDashPattern={[10]}
                />
