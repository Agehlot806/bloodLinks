...
const HospitalMap = ({ route }) => {
    ...
    const services_list = () => {
        hospitalboollist,
            LatLong
        console.log('Current location custom State and long list----->>>>>', currentLongitude, currentLatitude)
    ...
    const paramValuelat = parseFloat(route.params.LatNav);
    const paramValuelng = parseFloat(route.params.LongNav);
    const chibaRegion = {
        latitude: paramValuelat,
        longitude: paramValuelng,
        latitudeDelta: 0.04864195044303443,
        longitudeDelta: 0.040142817690068,
    };
    const getLocationApi = () => {
        fetch('https://bloodlinks.in/hospital_list')
            ...
            .then((response) => {
                console.log('response api data hospital Map list----->>>>>', response)
    ...
    const renderItem = ({ item }) => (
        ...
        <View style={styles.card}  >
            <View style={styles.textContent}>
                <View style={{ height: scale(90), width: scale(70), alignItems: 'center', justifyContent: 'center' }}>
                    <Image
                ...
                <View style={{ height: scale(80), width: '73%', padding: scale(5) }}>
                    <Text numberOfLines={2} style={styles.cardtitle}>{item.name}</Text>
                    <Text numberOfLines={4} style={styles.cardDescription}>{item.address_1}</Text>
                </View>
            </View>
